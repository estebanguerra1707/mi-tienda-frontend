...
} from "@/features/productos/productos.api";
...
const UNIDAD_CODES = UNIDADES.map((u) => u.code) as readonly UnidadCode[];
...
export default function ListPage() {
  ...
  const [productToEdit, setProductToEdit] = useState<EditableProduct | null>(
    null
  );
  ...
  const { data, isPending, error, refetch } = useAdvancedProducts(
    safeFiltro,
    pageUI - 1,
    size,
    {
  ...
  // Soporte scanner USB (Enter termina)
  useEffect(() => {
    let buffer = "";
    ...
    const handleKey = (ev: KeyboardEvent) => {
      if (ev.key === "Enter") {
        ...
        buffer = "";
        if (code.length > 2) setSearch(code);
        return;
      }
  ...
  // ✅ aquí queda lo importante: unidadMedidaCodigo consistente
    ...
    const unique: ProductListItem[] = Array.from(
      new Map(
        content.map((p) => {
          ...
          const rawCode = (p.unidadMedidaCodigo ?? "")
            .toString()
            .trim()
            .toUpperCase();

          const unidadCodigo: UnidadCode =
            (UNIDAD_CODES.includes(rawCode as UnidadCode)
              ? (rawCode as UnidadCode)
              : undefined) ?? "PIEZA";

          const unidadIdFromCode =
            UNIDADES.find((u) => u.code === unidadCodigo)?.id ?? UNIDADES[0].id;

          const unidadMedidaId = p.unidadMedidaId ?? unidadIdFromCode;
          ...
          const item: ProductListItem = {
            ...
            // compat (si EditProductButton aún la soporta)
            unidadMedidaId: Number(unidadMedidaId),

            // ✅ NUEVO: lo que usa el editor/PUT
  ...
  const isEmpty = !isPending && canShowProductsUI && sortedItems.length === 0;

  if (isPending && canFetchProducts) {
    return <p className="p-4">Cargando…</p>;
  }
  if (error) {
    return <p className="p-4 text-red-600">{(error as Error).message}</p>;
  }
        ...
        <>
                ...
                >
                  ...
                  <button
                    className="
                      ...
                      font-semibold
                      shadow-sm hover:shadow-md hover:bg-slate-200
                      transition
          ...
          >
            <ul className="grid grid-cols-1 gap-3 w-full max-w-full">
              {sortedItems.map((p) => (
                <li
                  key={`mobile-${p.id}-${p.branchId ?? 0}-${
                    p.inventarioOwnerType ?? "PROPIO"
                  }`}
                  onClick={() => openEdit(p)}
                  role="button"
                  tabIndex={0}
            ...
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-3 sm:p-6">
              <div className="bg-white rounded-3xl w-full max-w-md shadow-xl overflow-hidden">
                <div className="px-4 pt-4 pb-3 border-b">
                  ...
                  <p className="text-xs text-slate-500">
                    Alinea el código dentro de la cámara
                  </p>
                </div>
