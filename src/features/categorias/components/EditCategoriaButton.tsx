...
const baseSchema = z.object({
  ...
  businessTypeId: z.coerce.number().int().optional(),
});
...
export default function EditCategoriaButton({ id, onUpdated }: { id: number; onUpdated?: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        reset({
          name: found.name,
          businessTypeId: found.businessTypeId,
        });
        // no-super: refuerzo por si viniera vacío
        if (!isSuper && !found.businessTypeId && user?.businessType) {
          setValue("businessTypeId", user.businessType);
        }
      } catch (err) {
        setToast({ type: "error", message: getErrMsg(err) });
      }
    })();
    return () => { alive = false; };
  }, [open, id, reset, isSuper, setValue, user?.businessType]);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      await updateCategoria(id, {
        name: values.name,
        businessTypeId: (values.businessTypeId ?? user?.businessType ?? 0) as number,
      });
