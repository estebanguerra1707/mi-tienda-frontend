...
import {
  useProductsByBranch,
  type ProductsByBranchResponse,
} from "@/hooks/useProductsByBranch";
import { useAuth } from "@/hooks/useAuth";
import type { CompraCreate } from "@/features/compras/api";
import dayjs from "dayjs";
import "dayjs/locale/es";
import type { ResumenCompra, ProductoResumen } from "@/types/catalogs";
...
const makeZodResolver = <T extends object>(
  schemaParam: ZodSchema<T>
): Resolver<T> =>
  (zodResolver as unknown as (s: unknown) => unknown)(schemaParam) as Resolver<T>;
...
/**
 * Mantiene el caret al final SOLO mientras el input esté enfocado.
 * Útil cuando controlas el input por estado (value={...}).
 */
function useKeepCaretAtEnd(value: string) {
...
const normalizeProducts = (products: ProductsByBranchResponse | undefined): ProductItem[] => {
  if (!products) return [];
...
function formatByStep(n: number, step: number) {
  if (step === 1) return String(Math.trunc(n));
  if (step === 0.1) return n.toFixed(1);
  return n.toFixed(2);
}

/**
 * ✅ Componente controlado para quantity:
 * - Permite "", "1.", "1.1"
 * - No brinca el caret
 * - Normaliza en blur al step/min del producto
 */
function QuantityInput({
  ...
  unit: UnidadUI;
  onCommitNumber: (n: number) => void;
}) {
  const [local, setLocal] = useState<string>(
    valueNumber === undefined || valueNumber === null ? "" : String(valueNumber)
  );
  ...
  useEffect(() => {
    ...
    // Si el usuario está editando (ej. "1.") no lo pisamos.
    // Solo sincroniza si son idénticos o el local es "".
    if (local === "" || local === next) return;

    // Si el local termina en ".", lo dejamos.
    if (local.endsWith(".")) return;

    // Si el usuario está escribiendo decimales incompletos, no pisar.
    // (ej: "1.0" mientras el form trae 1)
    setLocal(next);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [valueNumber]);

  const caret = useKeepCaretAtEnd(local);
  ...
  return (
    <input
      ...
      onBlur={() => {
        ...
        const raw = local;
        if (raw === "" || raw === ".") {
          setLocal(String(unit.min));
          onCommitNumber(unit.min);
          return;
        }
        ...
        const clamped = Math.max(unit.min, stepped);

        // enteros si es pieza
        const finalN = unit.esPieza ? Math.trunc(clamped) : Number(formatByStep(clamped, unit.step));

        const text = formatByStep(finalN, unit.step);
        setLocal(text);
        onCommitNumber(Number(text));
      }}
      onChange={(e) => {
        const raw = sanitizeDecimal(e.target.value);

        // permite editar en vacío / punto
        if (raw === "" || raw === ".") {
          setLocal(raw);
          onCommitNumber(0);
          return;
        ...
        setLocal(raw);
        const n = Number(raw);
        onCommitNumber(Number.isNaN(n) ? 0 : n);
      }}
    />
  );
}
...
export default function AddCompraButton({ onCreated }: { onCreated?: () => void }) {
  ...
  const cashCaret = useKeepCaretAtEnd(localCash);
  ...
  const branches = useBranches({
    ...
    businessTypeId: isSuperAdmin ? undefined : user?.businessType ?? null,
    oneBranchId: isSuperAdmin ? null : user?.branchId ?? null,
  });

  const selectedBranch = branches.data?.find((b) => b.id === selectedBranchId);
  ...
  const getUnidadUI = useCallback((p: ProductItem): UnidadUI => {
    const label =
  ...
  // Para controlar quantity por índice sin romper reglas de hooks:
  ...
  /* ---------- Escaneo (barcode en input hidden) ---------- */
  useEffect(() => {
    ...
    if (found) {
      ...
      if (exists) {
        ...
        setValue("details", [...details, buildDetalle(found.id)], {
          shouldValidate: true,
        });
      }
      setValue("barcode", "");
  ...
  const onSubmit = async (values: CompraForm) => {
    if (!values.providerId || values.providerId === 0) {
      setToast({
        type: "error",
        message: "Selecciona un proveedor antes de continuar.",
      });
      return;
    }
    ...
    for (const d of values.details) {
      ...
      if (u.esPieza) {
        ...
        if (qty < u.min) {
          setToast({
            type: "error",
            message: `Cantidad inválida en "${prod.name}".`,
          });
          return;
        }
      } else {
    ...
    const resumen: ResumenCompra = {
      proveedor:
        providers.data?.find((p) => p.id === values.providerId)?.name ?? "",
  ...
  const handleBarcodeScan = async (code: string) => {
    ...
    if (exists) {
      ...
      setValue("details", [...details, buildDetalle(prod!.id)], {
        shouldValidate: true,
      });
    }
  ...
  /* ---------- Warning general ---------- */
  useEffect(() => {
    setShowWarning(Object.keys(errors).length > 0);
  }, [errors]);
  ...
  const productsList = useMemo(() => normalizeProducts(products), [products]);
        ...
        <DialogContent className="max-w-[90vw] md:max-w-5xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  {filteredProducts.map((p) => (
                    <div
                      ...
                      onClick={async () => {
                        ...
                        if (exists) {
                          ...
                          setValue("details", [...details, buildDetalle(p.id)], {
                            shouldValidate: true,
                          });
                        }
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500">
                        C.Barras: {p.barcode ?? p.codigoBarras ?? "N/A"} — SKU:{" "}
                        {p.sku} — ${(p.purchasePrice ?? 0).toFixed(2)}
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <table className="w-full text-sm border">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          {isSuperAdmin && (
                            <th className="text-center">Negocio</th>
                          )}
                      ...
                      <tbody>
                        {details.map((d, i) => {
                          const prod = productsList.find(
                            (p) => p.id === Number(d.productId)
                          );
                          if (!prod) return null;

                          const u = getUnidadUI(prod);
                          const qty = watchedDetails?.[i]?.quantity;
                                ...
                                <td className="w-40 text-center align-middle">
                                  <div className="flex items-center justify-center gap-2 min-w-[140px]">
                                    <input
                                      ...
                                      checked={
                                        (d.ownerType ?? "PROPIO") === "CONSIGNACION"
                                      }
                                      onChange={(e) => {
                                        setValue(
                                          `details.${i}.ownerType`,
                              ...
                              <td className="text-center">
                                <div className="flex justify-center gap-2 items-center">
                                  <button
                                    type="button"
                                    onClick={async () => {
                                      const current = Number(qty ?? d.quantity ?? u.min);
                                      const next = Math.max(
                                        u.min,
                                        Number((current - u.step).toFixed(2))
                                      );
                                      setValue(`details.${i}.quantity`, next, {
                                        shouldValidate: true,
                                        shouldDirty: true,
                                      });
                                      await trigger("details");
                                  ...
                                  <button
                                    type="button"
                                    onClick={async () => {
                                      const current = Number(qty ?? d.quantity ?? 0);
                                      const next = Number(
                                        (current + u.step).toFixed(2)
                                      );
                                      setValue(`details.${i}.quantity`, next, {
                                        shouldValidate: true,
                                        shouldDirty: true,
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2 items-center">
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  const current = Number(qty ?? d.quantity ?? 0);
                                  const next = Number((current + u.step).toFixed(2));
                                  setValue(`details.${i}.quantity`, next, {
                                    shouldValidate: true,
                                    shouldDirty: true,
                                  });
              ...
              <div className="flex flex-col items-start w-43">
                <FormSelect
                  ...
                  onChange={async (e) => {
                    ...
                    const isNowCash =
                      selected?.name?.toUpperCase() === "EFECTIVO";
                ...
                <div className="flex flex-col items-end gap-1 w-64">
                  <label className="flex flex-col gap-1 w-full">
                    ...
                    <input
                      ...
                      value={localCash}
                      onFocus={cashCaret.onFocus}
                      onBlur={() => {
                        cashCaret.onBlur();
                        const n = Number(localCash);
                        if (!localCash || Number.isNaN(n)) {
                          setLocalCash("0");
                        ...
                        setLocalCash(fixed);
                        setValue("cashGiven", Number(fixed), {
                          shouldValidate: false,
                        });
                      }}
                      onChange={(e) => {
            ...
            <div className="flex justify-end gap-2 border-t pt-3">
              ...
              <button
                type="submit"
                disabled={isPending || (isCash && Number(localCash) < totalCompra)}
